...
func (us *UploadStream) uploadChunks(ctx context.Context) error {
	...
	docs := make([]interface{}, 0) // we will use append to add new docs, so the initial slice just have capacity 0
	...
	for i := 0; i < us.bufferIndex; i += int(us.chunkSize) {
		...
		// This has lead to an "index out of range" error, so we change this code to the above
