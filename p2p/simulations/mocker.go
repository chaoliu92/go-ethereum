...
func probabilistic(net *Network, quit chan struct{}, nodeCount int) {
	...
	for {
		...
		for i := lowid; i < highid; i++ {
			...
			if err != nil {
				log.Error("ErrorMsg stopping node", "node", nodes[i])
				wg.Done()
				continue
			}
			go func(id enode.ID) {
				...
				err := net.Start(id)
				if err != nil {
					log.Error("ErrorMsg starting node", "node", id)
				}
				wg.Done()
			}(nodes[i])
		}
...
func connectNodesInRing(net *Network, nodeCount int) ([]enode.ID, error) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		if err != nil {
			log.Error("ErrorMsg creating a node!", "err", err)
			return nil, err
		}
		ids[i] = node.ID()
	}
	...
	for _, id := range ids {
		if err := net.Start(id); err != nil {
			log.Error("ErrorMsg starting a node!", "err", err)
			return nil, err
		}
	...
	for i, id := range ids {
		...
		if err := net.Connect(id, peerID); err != nil {
			log.Error("ErrorMsg connecting a node to a peer!", "err", err)
			return nil, err
		}
	}
