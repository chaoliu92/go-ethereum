...
type Tracer struct {
	...
	err error                  // ErrorMsg, if one has occurred
...
func New(code string) (*Tracer, error) {
	...
	if !tracer.vm.GetPropString(tracer.tracerObject, "step") {
		return nil, fmt.Errorf("Steps object must expose a function step()")
	}
	tracer.vm.Pop()

	if !tracer.vm.GetPropString(tracer.tracerObject, "fault") {
		return nil, fmt.Errorf("Steps object must expose a function fault()")
	}
	tracer.vm.Pop()

	if !tracer.vm.GetPropString(tracer.tracerObject, "result") {
		return nil, fmt.Errorf("Steps object must expose a function result()")
	}
	tracer.vm.Pop()
